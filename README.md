...
| [kali](kali) | 2.0 | Using [kalilinux/rolling](https://app.vagrantup.com/kalilinux/boxes/rolling), install pyenv, specified version of Python, specified pip and apt packages, checkout the specified git repos, pipx install the specified project, `curl|bash` install things, download and extract tarballs, install Java and maven with sdkman, and set some VM properties. Provisioning defaults to unprivileged, but this can be overriden. |
